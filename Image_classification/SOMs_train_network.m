...
function [weights_labels,weights]=SOMs_train_network(imgs_sample,imgs_sample_PCA,imgs_sample_labels)

% Set up the information of train data
train_data=imgs_sample_PCA;
num_images=size(train_data,2);
train_ClassLabel=imgs_sample_labels;
img_dim=[14 12];
...
for epoch = 1:num_epochs
    ...
    lr_n=learning_rate_initial*exp(-epoch/num_epochs);
...
for i = 1:num_neurons
    ...
    weights_labels(i)=train_ClassLabel(index);
    % Visualize the semantic map
    subplot(output_dim(1), output_dim(2), i);
    imshow(reshape(imgs_sample(:,index), [img_dim(1), img_dim(2)]));
